CREATE TABLE IF NOT EXISTS public.governance_public_audit_verifier_federation_packages (
  ...
  CONSTRAINT gpav_vf_pkg_scope_nonempty_chk CHECK (length(trim(package_scope)) > 0),
  CONSTRAINT gpav_vf_pkg_version_chk CHECK (length(trim(package_version)) > 0),
  CONSTRAINT gpav_vf_pkg_hash_chk CHECK (length(trim(package_hash)) > 0),
  CONSTRAINT gpav_vf_pkg_payload_obj_chk CHECK (jsonb_typeof(package_payload) = 'object'),
  CONSTRAINT gpav_vf_pkg_metadata_obj_chk CHECK (jsonb_typeof(metadata) = 'object'),
  CONSTRAINT gpav_vf_pkg_scope_enum_chk CHECK (
    package_scope IN ('verifier_federation_distribution')
  ),
  CONSTRAINT gpav_vf_pkg_batch_scope_hash_uniq UNIQUE (batch_id, package_scope, package_hash)
);
...
CREATE TABLE IF NOT EXISTS public.governance_public_audit_verifier_federation_package_signatures (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT gpav_vf_psig_scope_chk CHECK (
    package_scope IN ('verifier_federation_distribution')
  ),
  CONSTRAINT gpav_vf_psig_signer_key_chk CHECK (length(trim(signer_key)) > 0),
  CONSTRAINT gpav_vf_psig_algo_chk CHECK (length(trim(signature_algorithm)) > 0),
  CONSTRAINT gpav_vf_psig_sig_chk CHECK (length(trim(signature)) > 0),
  CONSTRAINT gpav_vf_psig_trust_domain_chk CHECK (length(trim(signer_trust_domain)) > 0),
  CONSTRAINT gpav_vf_psig_jurisdiction_chk CHECK (
    signer_jurisdiction_country_code IS NULL
    OR length(trim(signer_jurisdiction_country_code)) = 2
  ),
  CONSTRAINT gpav_vf_psig_channel_chk CHECK (length(trim(distribution_channel)) > 0),
  CONSTRAINT gpav_vf_psig_metadata_obj_chk CHECK (jsonb_typeof(metadata) = 'object'),
  CONSTRAINT gpav_vf_psig_uniq UNIQUE (package_id, signer_key, signature)
);
