...
- **Phase C.11 (activation demographic feed escalation on Governance)** — Same external execution page board counts **open or acknowledged** pages whose `page_key` matches **`activation_demographic_feed_worker_escalation`** (shared constant `GOVERNANCE_ACTIVATION_DEMOGRAPHIC_FEED_WORKER_ESCALATION_PAGE_KEY` in stewardship code, avoiding broader `activation_demographic_feed` matches if additional feed-scoped page keys are introduced later); stewardship deep link uses `#stewardship-activation-review` on the Activation stewardship card, and Governance admin scrolls smoothly to the matching hash when present.
...
- **Activation feed worker escalation policy (steward-managed)** — Default row in `activation_demographic_feed_worker_escalation_policies` drives freshness hours, minimum adapter issues, severity, and `escalation_enabled`; `activation_demographic_feed_worker_escalation_policy_summary`, `set_activation_demographic_feed_worker_escalation_policy`, and `act_feed_worker_esc_pol_evt_hist` expose read/update/history for feed stewards. `schedule_activation_demographic_feed_worker_jobs_impl` calls `maybe_escalate_activation_feed_worker_exec_page` with `use_policy_freshness: true`; steward manual refresh from the adapters panel passes `force_manual_escalation: true` with nullable freshness so operators can page ahead of autonomous thresholds (`20260504120000_…sql`, repair `20260504121000_…sql` for short internal function names on Postgres 63-byte limits).
...
- **Activation feed worker escalation incident history (steward-scoped RPC)** — `activation_demographic_feed_worker_escalation_page_history` returns recent `activation_demographic_feed_worker_escalation` external execution pages for activation feed stewards (`current_profile_can_manage_activation_demographic_feed_workers`) without requiring public-audit verifier manager permission; signed feed adapters panel surfaces 24h open/resolve flow, unresolved count, average resolution time, and a collapsible recent-page list (`20260502150000_…sql`).
- **Activation feed worker escalation in-panel incident ops** — `activation_demographic_feed_worker_escalation_page_board` lists pages for the latest (or requested) batch with the same narrow steward auth; `acknowledge_activation_demographic_feed_worker_escalation_page` and `resolve_activation_demographic_feed_worker_escalation_page` mirror the generic external execution acknowledge/resolve transitions but only for `activation_demographic_feed_worker_escalation` rows so feed stewards can clear incidents without verifier-manager RPCs; adapters panel shows current-batch open/ack pages with acknowledge/resolve actions (`20260502160000_…sql`).
- **Scheduler automation status escalation snapshot** — `activation_demographic_feed_worker_schedule_automation_status` now includes `worker_escalation_open_or_ack_page_count`, `worker_escalation_latest_opened_at`, and `worker_escalation_latest_page_severity` for the latest public audit batch so stewards can correlate pg_cron / ledger health with on-call paging pressure without a separate board query (`20260502170000_…sql`).
...
| **Roadmap §14 slice** (minimized trusted-backend + federation exchange; items 1–5 in §14) | **~76–82%** | Mirror/federation/proof distribution baselines are substantial; activation feed scheduling adds ledger + auto-resolve escalation loop, steward-scoped escalation history analytics, and in-panel acknowledge/resolve for feed-worker on-call pages; public audit external execution paging can optionally POST to steward-configured HTTPS webhooks via `pg_net` when enabled, while items 2–3 still need independent-worker / third-party distribution work described in `governance-implementation-roadmap-v0.1.md`. |
