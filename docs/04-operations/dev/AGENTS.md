...
- When the session topic is **Levela decentralization** (verifier mirror / federation rollout), end each assistant report with one **short program progress percentage** taken from `docs/04-operations/dev/verifier-federation/rollout-plan-v0.1.md` §9 (currently **~100%** for the §4 **implementation** row; **§10 field rehearsal** is a separate non-percentage gate—see that section). §9 figures are **not calculated**; update the doc when a substantive milestone or deliberate headline clarification warrants it; **do not** bump program % for copy-only or cosmetic-only changes.
