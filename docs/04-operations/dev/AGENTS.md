...
- When the session topic is **Levela decentralization** (verifier mirror / federation rollout), end each assistant report with one **short program progress percentage** taken from `docs/04-operations/dev/verifier-federation/rollout-plan-v0.1.md` §9 (currently **96%** for the §4 rollout). Update that doc when a substantive milestone moves the figure; **do not** bump program % for copy-only or cosmetic-only changes.
