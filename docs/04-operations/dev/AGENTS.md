...
- **Install and configuration default:** When a feature needs an install or environment setup (database extensions such as **pg_net** / **pg_cron**, migration apply, `.env` keys the agent can write, VPS paths the agent can reach over SSH), **perform it in-session** (migrations, `scripts/db/apply-remote-migration.sh`, documented fallbacks with `NOTICE` when privilege blocks). Do **not** hand those steps back to the user unless blocked after a direct attempt (for example extension requires `shared_preload_libraries` + restart only the operator can schedule, or vendor UI only the account owner can access)—then state the blocker and what was already tried.
